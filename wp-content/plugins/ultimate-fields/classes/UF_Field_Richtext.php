...
class UF_Field_Richtext extends UF_Field_Textarea {
	...
	function display_input() {
		...
		if( version_compare( $wp_version, '4.3', '>=' ) ) {
			echo '<div id="wp-' . $this->mce_id . '_id-wrap" class="wp-core-ui wp-editor-wrap tmce-active" data-mce-id="' . $this->mce_id . '">
			    <div id="wp-' . $this->mce_id . '_id-editor-tools" class="wp-editor-tools hide-if-no-js">
			        <div id="wp-' . $this->mce_id . '_id-media-buttons" class="wp-media-buttons">' . $media_buttons . '</div>
			            <div class="wp-editor-tabs">
			                <button type="button" id="' . $this->mce_id . '_id-tmce" class="wp-switch-editor switch-tmce" data-wp-editor-id="' . $this->mce_id . '_id">' . __( 'Visual' )  . '</button>
			                <button type="button" id="' . $this->mce_id . '_id-html" class="wp-switch-editor switch-html" data-wp-editor-id="' . $this->mce_id . '_id">' . __( 'Text' ) . '</button>
			            </div>
			        </div>

			        <div id="wp-' . $this->mce_id . '_id-editor-container" class="wp-editor-container">
			            <div id="qt_' . $this->mce_id . '_id_toolbar" class="quicktags-toolbar"></div>
			            <textarea class="wp-editor-area" rows="' . $this->rows . '" autocomplete="off" cols="40" name="' . $this->input_id . '" id="' . $this->mce_id . '_id">' . esc_html( $content ) . '</textarea>
			        </div>
			    </div>
			</div>';
