	...
	*/
	...
	_.extend( Field.prototype, {
		...
		construct: function( $item ) {
			...
			if( ! UF.isset( $item.size ) || ! $item.length ) {
				return;
			}
	...
	_.extend( UF.Container.prototype, {
		...
		getFields: function() {
			...
			this.$fields = this.element.find( '.uf-field:not(.uf-field .uf-field,.uf-separator)' );

			this.$fieldsDiv = this.element.is( '.uf-fields' )
				? this.element
				: this.element.find( '.uf-fields:eq(0)' );
		...
		initTabs: function() {
			...
			if( ! this.$fieldsDiv.children( '.uf-tabs' ).length ) {
				return;
			}

			// Get links and tabs
			this.tabNav = this.$fieldsDiv.find( '.uf-tab' );
			this.tabCnt = this.$fieldsDiv.children( '.tab' );
			...
			if( hash = location.hash.replace(/^#\//, '') ) {
				if( this.tabCnt.filter( '#' + hash ).length ) {
					// Show the tab from the hash
					this.showTab( hash.replace( /#/g, '' ), false );
				}
			} else {
				// Show the first tab
		...
		showTab: function( id, animate ) {
			...
			this.tabNav.removeClass( 'uf-tab-active' ).filter( '[href="#' + id + '"]' ).addClass( 'uf-tab-active' );
	...
	_.extend( UF.ContainerPostMeta.prototype, {
		...
		checkTemplates: function() {
			...
			if( ! this.$elements.template.length ) {
				return true;
			}
		...
		checkTerms: function() {
			...
			for( taxonomy in this.panelData.terms ) {
				...
				if( !this.$elements.terms[ taxonomy ].length ) {
					continue;
				}
	...
	Field.extend( 'Richtext', {
		...
		initMce: function( id, value ) {
			...
			$('#' + id + '_id').html( switchEditors._wp_Autop( value || that.values[ id ] || '' ) );

			// Setup the Richtext editor
			var mceInit = $.extend({}, tinyMCEPreInit.mceInit[ 'uf_dummy_editor_id' ], { body_class: id, elements: id, rows: $('#' + id + '_id').attr('rows'), selector: '#' + id + '_id' });
			...
			// Setup quicktags
			var qtInit = $.extend({}, tinyMCEPreInit.qtInit[ 'uf_dummy_editor_id' ], { id: id + '_id' });
			tinyMCEPreInit.qtInit[id + '_id'] = $.extend({}, qtInit);
			quicktags(tinyMCEPreInit.qtInit[id + '_id']);

			// Init QuickTags
			QTags._buttonsInit();

			// Listen to changes in textareas
			$('#' + id + '_id').on( 'change keyup', function(){
	...
	*/
	...
	_.extend( UF.RepeaterRow.prototype, {
		...
		getFields: function() {
			...
			if( $table.children( '.uf-field' ).length ) {
	...
	Field.extend( 'Repeater', {
		...
		rowDeleted: function( row ) {
			...
			if( ! this.$elements.fields.children( '.uf-row' ).length ) {
				this.$elements.placeholder.show();
			}

			// If limits have been reached, now there's space again
			if( this.$elements.prototypes.children().length > 1) {
