...
class UF_Tabs {
	...
	static private function get_link( $tab ) {
		...
		if( $icon ) {
			$icon = '<span class="uf-tab-icon">' . $icon . '</span>';
		}

		return '<a href="#' . $tab['id'] . '" class="uf-tab">' . $icon . '<strong class="uf-tab-text">' . esc_html( $tab[ 'title' ] ) . '</strong></a>';
	}
